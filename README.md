...
> **Measured results live in [`RESULTS.md`](RESULTS.md)**, generated by `bench/report.py`
> from the raw JSONL records of a declared, frozen run on clean source. Nothing in it is
> hand-entered. See `PROGRESS.md` for the per-milestone record.
...
the greedy baseline. The corpus is saturated at this budget, so it cannot discriminate between
the two generators — that limitation is stated in `RESULTS.md` rather than hidden.
